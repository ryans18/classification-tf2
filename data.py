...
class BingImagesSpider:
    ...
    def __init__(self, classes, amount, path='./'):
        # keyword: 需爬取的关键字
        # amount: 需爬取的数量
        # path: 图片存放路径
    ...
    def request_and_save_image(self, info):
        ...
        filename = '{}.{}'.format(info['index'], info['image_type'])
        filepath = os.path.join(info['path'], filename)
        try:
            # 请求图片
            response = requests.get(info['image_url'], headers=self.headers, timeout=3)
            # 保存图片
    ...
    def run(self):
        ...
        if os.path.exists(self.path)==False:
            os.mkdir(self.path)
        # 根据分类下载
        for keyword in self.classes:
            print('begin download: ', keyword);
            self.download(keyword)
            print('end download: ', keyword);
    ...
    def download(self, keyword):
        ...
        for i in range(int(self.amount/self.per_page_images * 1.5) + 1): # 由于有些图片会重复，故先请求1.5倍图片，豁免
            url = self.bing_image_url_pattern.format(keyword, i*self.per_page_images, self.per_page_images)
...
if __name__ == '__main__':
    ...
    # 需要的图片数量：100
    # 图片保存路径：'E:\images'
    class_path = "images\cls_classes.txt"
    pic_size = 50
    image_path = "images"
    start = time()
    classes, num_classes = get_classes(class_path)
    BingImagesSpider(classes, pic_size, image_path).run()
    print(time() - start)